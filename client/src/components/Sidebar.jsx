...
const Sidebar = ({  isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    ...
    ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      {/* New Chat Button */}
      <button
        className="flex justify-center items-center w-full py-2 mt-5 text-white bg-gradient-to-r from-[#A456F7]
      ...
      <div className='flex items-center gap-2 p-3 mt-4 border border-gray-400 dark:border-white/20 rounded-md'>
        ...
        type="text" placeholder='Search conversations' className='text-xs placeholder:text-gray-400 outline-none'/>
        </div>
        ...
        <div className="flex-1 overflow-y-scroll mt-3 text-sm space-y-3">
          {
            chats.filter((chat)=> chat.messages[0] ? chat.messages[0]?.content.toLowerCase().includes(search.toLocaleLowerCase()) :
          chat.name.toLocaleLowerCase().includes(search.toLowerCase())).map((chat)=>(
            <div onClick={()=>{navigate('/'); setSelectedChat(chat); setIsMenuOpen(false)}}
            key={chat._id} className="p-2 px-4 dark:bg-[#57317C]/10 border border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer
            flex justify-between group">
              <div>
                <p className="truncate w-full">
                        {chat.messages.length > 0 ? chat.messages[0].content.slice(0,32) : chat.name}
                </p>
                <p className="text-xs text-gray-500 dark:text-[#81A6C0]">{moment(chat.updatedAt).fromNow()}</p>
              </div>
        ...
        className="flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all">
          ...
          <div className="flex flex-col text-sm">
            ...
            <p className="text-xs text-gray-400">Purchase credits to use ThinkSy</p>
          </div>
        </div> 
